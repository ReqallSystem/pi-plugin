...
test('npm tarball ships policy and seven aligned skills and loads through Pi', async () => {
  ...
  try {
    ...
    assert.ok(recordSchema.properties.kind.enum.includes('work'));
    assert.ok(recordSchema.properties.kind.enum.includes('info'));
    assert.equal(recordSchema.properties.session_id, undefined, 'origin is transport-owned, not a model argument');
    assert.match(readFileSync(join(root, 'PARITY.md'), 'utf8'), /Remaining gaps/);
